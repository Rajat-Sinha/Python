...
def accept_connections():
    ...
    while 1:
        try:
            ...
            conn.setblocking(1) #blocking to 1 I don't want any timeout
...
#Display all the current Connection
def list_connections():
    #enumerate is used for using counter variable
    results = ''
...
def create_workers():
    for _ in range(NUMBER_OF_THREAD):
        ...
        t = threading.Thread(target=work) #target is basiclly what function you want to run
        #thread t is doing job work
        t.daemon = True #this thread whichc is basically a mini program is die when our main program exits
        t.start()
